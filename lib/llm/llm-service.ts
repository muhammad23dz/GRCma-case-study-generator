...
export async function query(prompt: string, systemPrompt?: string): Promise<string> {
    ...
    if (!apiKey) {
        console.error(`[LLM Service] No API key found for provider '${provider}'`);
        throw new Error(`No API key configured for AI provider. Please set DEEPSEEK_API_KEY, OPENAI_API_KEY, or GITHUB_TOKEN.`);
    }
